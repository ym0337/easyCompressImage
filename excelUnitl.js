...
if (_data) {
  // data.shift(); // 删除第一列表头
  // 移除置灰的数据
  data = _data.filter(item => item.isActive != '置灰')
...
// 创建二级节点
const dom2 = createDom('一级菜单', data, '业务域');
// 创建三级节点
const dom3 = createDom('二级菜单', data, '一级菜单');
// 创建四级节点
const dom4 = createDom('三级菜单', data, 'uuid');
  ...
  DomObj.forEach(item2 => {
    if (item2.name === item['业务域']) {
      item2.list.push({
        ...
        '一级菜单': item['一级菜单'],
        name: item['一级菜单'],
        defaultIcon: item['defaultIcon一级菜单'],
        activeIcon: item['activeIcon一级菜单'],
...
function createDom (type, data, uuid = '') {
  const seen = new Set();
  const uniqueArray = [];
  data.filter(item => {
    const keyValue = uuid ? item[type] + item[uuid] : item[type];
